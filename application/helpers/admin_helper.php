<?php
define('UPDATE_URL', 'https://update.arabcode.com/');
...
function btn_delete($uri, $text = null, $icon = null)
{
    ...
    return anchor($uri, $icons . ' ' . $title, array(
        'class' => "btn $btn-danger btn-xs", 'title' => $text, 'data-toggle' => 'tooltip', 'data-placement' => 'top', 'onclick' => "return confirm('" . lang('delete_alert') . "');"
    ));
}
...
function get_permission($colL, $col, $permission_user, $permissionL = null, $text = null)
{
    ...
    $html .= '" id="permission_user_1"><label for="field-1" class="col-sm-' . $colL . ' control-label">' . lang('select') . ' ' . lang('users') . '<span class="required">*</span></label>
<div class="col-lg-' . $col . '">';
    if (!empty($permission_user)) {
        foreach ($permission_user as $key => $v_user) {
            ...
            $html .= '<div class="checkbox c-checkbox needsclick"><label class="needsclick">
                <input class="needsclick" type="checkbox" name="assigned_to[]" value="' . $v_user->user_id . '"';
            if (!empty($permissionL) && $permissionL != 'all') {
...
<div class="col-lg-' . $col . '">';
    if (!empty($permission_user)) {
        foreach ($permission_user as $key => $v_user) {
            ...
            $html .= ' " id="action_' . $v_user->user_id . '"><label class="checkbox-inline c-checkbox"><input id="' . $v_user->user_id . '" checked type="checkbox" name="action_' . $v_user->user_id . '[]" disabled value="view"> <span class="fa fa-check"></span>' . lang('can') . ' ' . lang('view') . '</label>';
            $html .= '<label class="checkbox-inline c-checkbox"><input ' . (!empty($disable) ? 'disabled' . ' ' . 'checked' : ' ') . ' id="' . $v_user->user_id . '"  type="checkbox" name="action_' . $v_user->user_id . '[]" value="edit"';
            if (!empty($permissionL) && $permissionL != 'all') {
                foreach ($user_id as $uid => $v_permission) {
                    if ($uid == $v_user->user_id) {
...
function get_staff_details($user_id = null, $type = null)
{
    $CI = &get_instance();
    $CI->db->select('tbl_users.*', FALSE);
    $CI->db->select('tbl_account_details.*', FALSE);
...
function get_form_field($field, $colLabel = 'col-lg-3', $col = 'col-lg-7')
{
    $value = explode('|', $field);
    $cfields = '';
    if (!empty($value[0])) {
        ...
        if (!empty($value[1])) {
            if ($value[1] == 'password') {
                ...
                $cfields .= '<input type="' . $value[1] . '" name="' . $value[0] . '" class="form-control" value="' . $password . '" required>';
                $cfields .= '<strong id="show_password" class="required"></strong>';
                $cfields .= '</div>';
                $cfields .= '<div class="col-lg-2">';
                $cfields .= '<a data-toggle="modal" data-target="#myModal" href="' . base_url('admin/client/see_password/paypalpassword') . '" id="see_password">' . lang('see_password') . '</a>';
